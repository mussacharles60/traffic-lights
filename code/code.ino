...
uint8_t last_selected_distance = 0;
...
void longTrafficDetection() {
  if (should_select_distance) {
    ...
    // }
    if (last_selected_distance == 0 || last_selected_distance == 3) { // last measured was distance C or there was not done any measurements
      if (distance_A <= SHORT_DISTANCE_TRAFFIC) {
        should_select_distance = false;
        distance_to_measure = 1; // A
        last_selected_distance = 1;
      } else if (distance_B <= SHORT_DISTANCE_TRAFFIC) {
        should_select_distance = false;
        distance_to_measure = 2; // B
        last_selected_distance = 2;
      } else if (distance_C <= SHORT_DISTANCE_TRAFFIC) {
        should_select_distance = false;
        distance_to_measure = 3; // C
        last_selected_distance = 3;
      }
    } else if (last_selected_distance == 1) { // last measured was distance A
      if (distance_B <= SHORT_DISTANCE_TRAFFIC) {
        should_select_distance = false;
        distance_to_measure = 2; // B
        last_selected_distance = 2;
      } else if (distance_C <= SHORT_DISTANCE_TRAFFIC) {
        ...
        distance_to_measure = 3; // C
        last_selected_distance = 3;
      } else if (distance_A <= SHORT_DISTANCE_TRAFFIC) {
        should_select_distance = false;
        distance_to_measure = 1; // A
        last_selected_distance = 1;
      }
    } else if (last_selected_distance == 2) { // last measured was distance B
      if (distance_C <= SHORT_DISTANCE_TRAFFIC) {
        should_select_distance = false;
        distance_to_measure = 3; // C
        last_selected_distance = 3;
      } else if (distance_A <= SHORT_DISTANCE_TRAFFIC) {
